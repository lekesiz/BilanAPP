...
async function deductCredits(userId, amount) {
    ...
    try {
        ...
        if (user.availableCredits < amount) {
            return { success: false, message: `Yetersiz kredi. Bu işlem için ${amount} kredi gereklidir. Mevcut kredi: ${user.availableCredits}` };
        }
        ...
        return { success: false, message: 'Kredi düşülürken bir sunucu hatası oluştu.' };
    }
}
 ...
 */
async function logCreditChange(userId, amount, action, description = null, adminUserId = null, relatedResourceId = null, relatedResourceType = null) {
    try {
        // Admin ID'sini açıklamaya ekleyebiliriz
        let finalDescription = description;
        if (adminUserId) {
            finalDescription = description ? `${description} (par Admin ID: ${adminUserId})` : `Ajustement par Admin ID: ${adminUserId}`;
        }

        await CreditLog.create({
            userId,
            amount: amount, // Pozitif veya negatif olabilir
            action,
