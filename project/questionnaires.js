...
router.get('/', ensureAuthenticated, async (req, res) => {
  try {
    ...
    req.flash(
      'error',
      'Une erreur est survenue lors du chargement des questionnaires',
    );
...
router.post(
  ...
  async (req, res) => {
    try {
      const {
        beneficiaryId, title, description, type, dueDate,
      } = req.body;
      ...
      req.flash(
        'error',
        'Une erreur est survenue lors de la création du questionnaire',
      );
...
router.post('/:id/complete', ensureAuthenticated, async (req, res) => {
  try {
    ...
    if (req.user.userType !== 'beneficiary') {
      req.flash(
        'error',
        'Seuls les bénéficiaires peuvent compléter les questionnaires',
      );
      return res.redirect('/questionnaires');
    }
