...
CreditLog.init(
  {
    userId: {
      ...
      onDelete: 'SET NULL', // Kullanıcı silinirse log kalsın, user ID null olsun
    },
    action: {
      // Yapılan işlem (örn: ANKET_ATAMA, DOC_YUKLEME)
      type: DataTypes.STRING,
      allowNull: false,
    },
    amount: {
      // Harcanan (-) veya eklenen (+) kredi miktarı
      type: DataTypes.INTEGER,
      allowNull: false,
    },
    description: {
      // İşlem açıklaması (örn: Anket 'XYZ' atandı)
      type: DataTypes.STRING,
      allowNull: true,
    },
    relatedResourceId: {
      // İlişkili kaynağın ID'si (örn: Questionnaire ID)
      type: DataTypes.INTEGER,
      allowNull: true,
    },
    relatedResourceType: {
      // İlişkili kaynağın türü (örn: 'Questionnaire')
      type: DataTypes.STRING,
      allowNull: true,
    },
    adminUserId: {
      // İşlemi yapan adminin ID'si
      type: DataTypes.INTEGER,
