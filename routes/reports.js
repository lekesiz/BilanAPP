...
router.get('/', 
    ...
    try {
        ...
        const ownBeneficiaryIds = (await Beneficiary.findAll({ where: { consultantId }, attributes: ['id']})).map(b => b.id);
        const overdueQuestionnairesCount = ownBeneficiaryIds.length > 0 ? await Questionnaire.count({
             where: {
        ...
        // 5. Eksik Rıza/Sözleşme Sayıları (getConsultantStats'tan da alınabilir ama burada ayrı hesaplayalım)
         const missingConsentsCount = await Beneficiary.count({ where: { consultantId, consentGiven: false } });
         const missingAgreementsCount = await Beneficiary.count({ where: { consultantId, agreementSigned: false } });
