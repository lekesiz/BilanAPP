...
describe('Beneficiary Routes', () => {
  ...
  it('POST /beneficiaries/add - Should fail without required fields', async () => {
    ...
    const res = await agent.post('/beneficiaries/add').send({ _csrf: csrfToken });
  ...
  it('POST /beneficiaries/add - Should add a new beneficiary successfully', async () => {
    ...
    const res = await agent.post('/beneficiaries/add').send(newBeneficiaryData);
  ...
  it('POST /beneficiaries/:id/edit - Should update the beneficiary', async () => {
    ...
    const res = await agent.post(`/beneficiaries/${createdBeneficiaryId}/edit`).send(updatedData);

    expect(res.statusCode).toEqual(302);
    expect(res.headers.location).toEqual(`/beneficiaries/${createdBeneficiaryId}`);

    // Verify update in DB
    const updatedBeneficiary = await Beneficiary.findByPk(createdBeneficiaryId, {
      include: 'user',
    });
